...
- `local-path` claims are hostPath directories, so the kubelet never emits `kubelet_volume_stats_*` for them. `infrastructure/storage/local-path-provisioner/usage-exporter.yaml` runs a busybox DaemonSet that walks `/var/mnt/local-path-provisioner` (primary) and `/var/lib/local-path-provisioner` (utility node) with `du` every five minutes and writes `local_path_pvc_used_bytes{namespace,persistentvolumeclaim,volumename,node}` to `/var/mnt/node-exporter-textfile/local_path_pvc.prom`. node-exporter serves it through `--collector.textfile.directory`. The `homelab:local_path_pvc_*` rules join on `volumename` so orphaned directories from deleted PVs are ignored, and capacity is the claim's requested size because local-path enforces no quota. `local_path_pvc_exporter_last_run_timestamp_seconds` per node is the staleness signal.
