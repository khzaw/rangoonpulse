...
`controlpanel.khzaw.dev/#deploy` provides one-click Flux image-automation reconciliation for the self-built site targets: `blog`, `mmcal`, `ericaknight`, `itvp`, `retirement`, and `rangoon-mapper`. Each site button runs only that site's image automation. The global **Deploy all** button runs the same targeted flow for every site in sequence. Use it after the source repository push has finished publishing the new GHCR image and waiting for the normal registry scan/write cadence is inconvenient.
